...
const DeliveredOrdersPage = () => {
  ...
  useEffect(() => {
    if (Array.isArray(ordersDelivered.data)) {
      ...
      console.log('ordersDelivered', ordersDelivered.data);
    } else {
      console.log('ordersDelivered data is not an array or is undefined');
    }
  ...
  const handleFilterData = (e) => {
    ...
    if (!ordersDelivered?.data || !Array.isArray(ordersDelivered.data)) {
      console.error('Invalid orders data:', ordersDelivered.data);
      return;
    }

    if (text === '') {
      setFilteredOrders(ordersDelivered.data); // Reset if input is empty
    } else {
      console.log('Filtering for text:', text);

      const filter = ordersDelivered.data.filter((order) =>
