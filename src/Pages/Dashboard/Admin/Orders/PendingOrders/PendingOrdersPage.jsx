...
const PendingOrdersPage = () => {
  ...
  const handleFilterData = (e) => {
    ...
    if (!ordersPending?.data || !Array.isArray(ordersPending.data)) {
      console.error('Invalid orders data:', ordersPending.data);
      return;
    }

    if (text === '') {
      setFilteredOrders(ordersPending.data); // Reset if input is empty
    } else {
      console.log('Filtering for text:', text);

      const filter = ordersPending.data.filter((order) =>
