...
const CanceledOrdersPage = () => {
  const ordersCanceled = useSelector((state) => state.ordersCanceled);
  ...
  const handleFilterData = (e) => {
    ...
    if (!ordersCanceled?.data || !Array.isArray(ordersCanceled.data)) {
      console.error('Invalid orders data:', ordersCanceled.data);
      return;
    }

    if (text === '') {
      setFilteredOrders(ordersCanceled.data); // Reset if input is empty
    } else {
      console.log('Filtering for text:', text);

      const filter = ordersCanceled.data.filter((order) =>
