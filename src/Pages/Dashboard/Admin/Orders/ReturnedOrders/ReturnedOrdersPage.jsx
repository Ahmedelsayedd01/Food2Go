...
const ReturnedOrdersPage = () => {
  ...
  const handleFilterData = (e) => {
    ...
    if (!ordersReturned?.data || !Array.isArray(ordersReturned.data)) {
      console.error('Invalid orders data:', ordersReturned.data);
      return;
    }

    if (text === '') {
      setFilteredOrders(ordersReturned.data); // Reset if input is empty
    } else {
      console.log('Filtering for text:', text);

      const filter = ordersReturned.data.filter((order) =>
